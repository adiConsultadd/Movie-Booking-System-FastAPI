...
@router.get(
    "/movies", response_model=List[ViewAllMovies], status_code=status.HTTP_200_OK
)
def get_movies(
    db: Annotated[Session, Depends(get_db)], user: dict = Depends(is_authenticated)
):
...
@router.post(
    "/movies/{movie_id}/book",
    response_model=BookingDone,
...
def book_ticket(
    ...
    existing_booking = (
        db.query(Booking)
        .filter(Booking.movie_id == request.movie_id, Booking.user_id == user["id"])
        .first()
    )
    if existing_booking:
...
def cancel_booking(
    ...
    booking = (
        db.query(Booking)
        .filter(Booking.movie_id == movie_id, Booking.user_id == user.get("id"))
        .first()
    )
    if not booking:
